...
/**
 * Async movement: same forward-input loop as NavigateTo but with no
 * auto-jump — strictly walks (and sprints when speed > 1.0). Use this when
 * the caller has already proven the path is flat.
 */
public final class PathfindToAction implements ActionExecutor {
    ...
    public ActionResult execute(JsonObject params) {
        ...
        SkillTask task = engine.create("pathfind_to");
        ...
        State st = new State(injected, tx, ty, tz, speed, timeout, task.taskId());
    ...
    private static final class State implements Runnable {
        ...
        State(InjectedInput in, double tx, double ty, double tz,
              double speed, int timeout, String taskId) {
            this.injected = in;
            this.tx = tx; this.ty = ty; this.tz = tz;
            this.speed = speed; this.timeout = timeout; this.taskId = taskId;
        }
