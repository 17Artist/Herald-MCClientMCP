...
/** Port of BlackBoxPro advanced/EditBookAction.kt. */
public final class EditBookAction implements ActionExecutor {

    @Override
    public ActionResult execute(JsonObject params) {
        int slot = JsonUtil.requireInt(params, "slot");
        JsonArray pagesArr = JsonUtil.getArrayOrEmpty(params, "pages");
        String title = JsonUtil.getStringOrDefault(params, "title", null);
        ...
        for (int i = 0; i < pagesArr.size(); i++) {
            ...
            if (p.length() > 32767) {
                return ActionResult.error(ErrorCode.INVALID_PARAMS, "Page " + i + " too long");
            }
            pages.add(p);
        }
        ClientPacketListener conn = McHelper.connection();
        if (conn == null) return McHelper.notConnected();
        conn.send(new ServerboundEditBookPacket(slot, pages, title != null ? Optional.of(title) : Optional.empty()));
