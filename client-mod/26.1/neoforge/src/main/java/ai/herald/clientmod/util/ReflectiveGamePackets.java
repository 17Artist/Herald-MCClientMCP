...
public final class ReflectiveGamePackets {
    ...
    public static ActionResult sendCustomPayload(ClientPacketListener conn, Object channel, byte[] data) {
        try {
    ...
    private static Object newCustomPayloadPacket(Object channel, byte[] data)
            throws ReflectiveOperationException {
        Class<?> gamePkt = tryLoad("net.minecraft.network.protocol.game.ServerboundCustomPayloadPacket");
        if (gamePkt != null) {
            FriendlyByteBuf buf = new FriendlyByteBuf(Unpooled.wrappedBuffer(data));
            Constructor<?> ctor = gamePkt.getConstructor(Object.class, FriendlyByteBuf.class);
    ...
    private static final class RawCustomPayloadHandler implements InvocationHandler {
        ...
        RawCustomPayloadHandler(Object id, byte[] data) {
            this.id = id;
            this.data = data;
        }
