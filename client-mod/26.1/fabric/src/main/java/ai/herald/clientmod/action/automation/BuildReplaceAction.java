...
/**
 * Sync: replaces one block type with another in a region using /fill ... replace.
 */
public final class BuildReplaceAction implements ActionExecutor {
    ...
    public ActionResult execute(JsonObject params) {
        ...
        String fromBlock = JsonUtil.requireString(params, "fromBlock");
        String toBlock = JsonUtil.requireString(params, "toBlock");
        ...
        if (volume <= MAX_FILL_VOLUME) {
            ...
            for (int cy = minY; cy <= maxY; ) {
                ...
                player.connection.sendCommand(
                    "fill " + minX + " " + cy + " " + minZ + " " + maxX + " " + endY + " " + maxZ
                        + " " + toBlock + " replace " + fromBlock);
                commandsSent++;
                cy = endY + 1;
            }
